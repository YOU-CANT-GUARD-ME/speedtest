...
async function loadData() {
    const rows = ((await (fetch('sample-data.csv'))).text())
    .split('\n')
    .map(r => r.split(','));
...
previousButton.onclick = () => (currentPage--, loadData());
nextButton.onclick = () => (currentPage++, loadData());

pageButton.forEach((b, i) => {
    b.onclick = () => (currentPage = i + 1, loadData())
});
